...
#include <utils.h>
    ...
    #include <cm/cm.h>
#else
    #define SNPRINT_FN_UNDER_TEST kearly_snprintf
    #include <kdebug.h>
#endif // LIBCM
...
TEST(snprintf, unsigned_int_bit_format)
{
    INT num = 0xFF01;
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%b", num);
    EQ_SCALAR(ret, 17);
...
TEST(snprintf, unsigned_int_decimal_format)
{
    INT num = 1045;
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%u", num);
    EQ_SCALAR(ret, 4);
    EQ_STRING(d, "1045");
    END();
}
...
TEST(snprintf, unsigned_int_hex_format_without_base_identifier)
{
    INT num = 0x123def;
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%h", num);
    EQ_SCALAR(ret, 6);
    EQ_STRING(d, "123DEF");
    END();
}
...
TEST(snprintf, unsigned_int_hex_format)
{
    INT num = 0x123def;
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%x", num);
    EQ_SCALAR(ret, 7);
    EQ_STRING(d, "123DEFh");
    END();
}
...
TEST(snprintf, unsigned_int_octal_format)
{
    INT num = 07210;
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%o", num);
    EQ_SCALAR(ret, 5);
    EQ_STRING(d, "7210o");
    END();
}
...
TEST(snprintf, unsigned_long_hex_format)
{
    U32 num = 0xCF010203;
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%lx", num);
    EQ_SCALAR(ret, 9);
...
TEST(snprintf, unsigned_long_long_hex_format)
{
    ...
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%llx", num);
    EQ_SCALAR(ret, 17);
...
TEST(snprintf, limit_check_empty_string)
{
    CHAR *message = "";
    CHAR d[1];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%s", message);
    EQ_SCALAR(ret, 0);
    EQ_STRING(d, "");
    END();
}
...
TEST(snprintf, limit_check_overflow_mixed)
{
    INT num = 0xFF012EA;
    CHAR d[15];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "Hello %s %x",
                                                   "World", num);
    EQ_SCALAR(ret, 20);
...
{
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "100%%");
    EQ_SCALAR(ret, 4);
    EQ_STRING(d, "100%");
    END();
}
...
TEST(snprintf, wrong_format)
{
    ...
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "Hello %arjob");
    EQ_SCALAR(ret, 12);
...
{
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "Arjob 0x10A");
    EQ_SCALAR(ret, 11);
...
{
    CHAR d[MAX_PRINTABLE_STRING_LENGTH];
    INT ret1 = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "Hello");
    INT ret2 = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%s World", d);
    EQ_SCALAR(ret1, 5);
...
TEST(snprintf, pointer_literal)
{
    ...
    CHAR d[10];
    INT ret = SNPRINT_FN_UNDER_TEST (d, ARRAY_LENGTH(d), "%px", num);
    EQ_SCALAR(ret, 8);
