...
#define ALIGN_UP(a, n)   (IS_ALIGNED((a), (n)) ? (a) : (((a) / (n)) + 1) * (n))

#ifdef DEBUG
    #define PRINT(fnt, ...) printf("\n[%s]: " fnt, __func__, ##__VA_ARGS__)
#else
...
static MallocHeader *createNewNode(void *at, size_t netSize);
static MallocHeader *findFirst(KMallocLists list, FindCriteria criteria, uint32_t value);
static MallocHeader *getMallocHeaderFromList(KMallocLists list, ListNode *node);
static ListNode *getListHead(KMallocLists list);
static void splitFreeNode(size_t bytes, MallocHeader *freeNodeHdr);
static void combineAdjFreeNodes(MallocHeader *node);
...
void kmalloc_init() {
    ...
    MallocHeader *newH = createNewNode(buffer, INITIAL_HEAP_SIZE);
...
void *kmalloc(size_t bytes) {
    ...
    MallocHeader *node     = findFirst(FREE_LIST, FIND_CRIT_NODE_SIZE, searchAllocSize);

    if (node != NULL) {
        // Split the free node into two.
        assert(node->netNodeSize >= netAllocSize);
        splitFreeNode(bytes, node);
...
void kfree(void *addr) {
    ...
    MallocHeader *allocHdr = findFirst(ALLOC_LIST, FIND_CRIT_NODE_ADDRESS,
...
static void combineAdjFreeNodes(MallocHeader *node) {
    ...
    assert(current->isAllocated == false);
...
static ListNode *getListHead(KMallocLists list) {
    switch (list) {
    case FREE_LIST: return &freeHead;
    case ALLOC_LIST: return &allocHead;
    case ADJ_LIST: return &adjHead;
...
static MallocHeader *getMallocHeaderFromList(KMallocLists list, ListNode *node) {
    switch (list) {
    case FREE_LIST: return LIST_ITEM(node, MallocHeader, freenode);
    case ALLOC_LIST: return LIST_ITEM(node, MallocHeader, allocnode);
    case ADJ_LIST: return LIST_ITEM(node, MallocHeader, adjnode);
    default: assert(false);
    };
}
...
static MallocHeader *findFirst(KMallocLists list, FindCriteria criteria, uint32_t value) {
    ...
    list_for_each(head, node) {
        header = getMallocHeaderFromList(list, node);
...
static void splitFreeNode(size_t bytes, MallocHeader *freeNodeHdr) {
    ...
    assert(remainingSize >= sizeof(MallocHeader));

    MallocHeader *newFreeNodeHdr = createNewNode((void *)splitAt, remainingSize);
